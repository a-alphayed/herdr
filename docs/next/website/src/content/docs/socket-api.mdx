...
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send`, `agent.submit`, `agent.rename`, `agent.focus`, `agent.start`, `agent.teardown` |
...
omitted. `pane.move` always requires the source `pane_id`. `pane.split` is
federation-routable for host-qualified `target_pane_id` or `workspace_id`
selectors (`<host>/terminal:<id>`, `<host>/pane:<id>`, or
`<host>/workspace:<id>`) when the remote advertises the `pane_split`
federation capability; the controller resolves live projection ids and forwards
the existing request to the authoritative remote host.
...
`pane_id`, `label`, `cwd`, argv `command`, and `terminal_id` (the server-assigned
transient terminal id; omitted by older remotes). Split nodes use
