...
Neither command performs any remote lifecycle or destructive action. They do not start, stop, restart, install, or update the remote Herdr server; they do not kill processes, close panes, or delete remote state. Runtime bridge lifecycle commands such as `remote connect`/`reconnect`/`disconnect` are not implemented yet — `remote add`/`remove` only edit local config.
