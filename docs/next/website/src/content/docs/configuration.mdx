...
Runtime bridge lifecycle commands such as `remote connect`/`reconnect`/`disconnect` (ensure/teardown/reopen local aggregation bridges) are a separate future item and are not implemented yet; `remote setup` prepares a host's Herdr for use and is distinct from runtime bridge lifecycle.
