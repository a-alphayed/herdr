...
`host_rail_width` sets the width in columns of the hosts rail beside the Spaces/Agents panel. The default is 12, and the value is clamped to 9-24. You can also drag the rail's right edge to resize it, or double-click that edge to reset it to the configured width; the dragged width is remembered across restarts.
