...
Configured remote hosts define named aliases used by federated/host-qualified remote API controls (remote status/check, agent/pane/tab/workspace routed operations) and configured-host terminal attach. `herdr --remote <target>` at the root still takes a raw SSH target and does not resolve these aliases.
...
`connect_timeout_secs` bounds the SSH `ConnectTimeout` (whole seconds) used for connection attempts to this host, for both interactive and noninteractive configured-host SSH invocations. Optional; defaults to 10 seconds. Must be between 1 and 300 seconds inclusive; an out-of-range value fails config validation.
