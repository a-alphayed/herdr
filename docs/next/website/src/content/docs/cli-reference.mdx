...
calling pane, and an omitted target keeps using the UI-focused pane. A
host-qualified split target such as `jafar/terminal:<id>`, `jafar/pane:<id>`,
or `jafar/workspace:<id>` routes the existing split operation to the owning
remote Herdr session when that host advertises the `pane_split` federation
capability; `--cwd` and `--env` are interpreted on the remote host.
...
herdr agent submit <target> <text>
...
herdr agent teardown <target> --confirm
...
`agent read` reads the resolved terminal stream. `agent send` writes literal text to that stream. `agent submit` writes the text followed by an encoded Enter so composer-style agent prompts are actually submitted without an interactive attach; use it for headless/controller-side prompt handoff, and use `agent send` when you only want to inject text. `agent get`, `agent focus`, `agent wait`, and `agent attach` resolve the terminal via `agent.get` and require it to have agent identity, except that `agent attach <host>/terminal:<id>` bypasses `agent.get` for direct terminal attach using a server-assigned terminal id from `layout.export`. `agent rename` can assign that identity.

`agent teardown <target> --confirm` is a narrow, explicitly destructive, capability-gated close of the pane hosting a federation-placed agent/lane. `--confirm` is required and the request is rejected before any local close or remote forwarding when it is missing; `confirm: true` does not bypass the worktree-group close guard. Host/session-qualified remote targets (`<host>/<target>`) require the owning host to advertise the `agent_teardown` capability and are forwarded as `agent.teardown` against the resolved terminal id. It is not broad pane/workspace/server/process management, and non-agent terminal/pane ids fail as an agent-target error rather than closing a pane.
...
Use `pane send-text`, `pane send-keys`, `pane run`, and `terminal attach` for ordinary terminals, servers, tests, shells, or low-level terminal control. Use `pane run` when you want to submit a command with Enter on a plain shell; use `agent submit` for composer-style agent prompt submission.
