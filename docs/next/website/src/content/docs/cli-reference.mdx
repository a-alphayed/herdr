...
`agent read` reads the resolved terminal stream. `agent send` writes literal text to that stream. `agent submit` writes the text followed by an encoded Enter so composer-style agent prompts are actually submitted without an interactive attach; use it for headless/controller-side prompt handoff, and use `agent send` when you only want to inject text. `agent get`, `agent focus`, `agent wait`, and `agent attach` require the resolved terminal to have agent identity. `agent rename` can assign that identity.
