...
- Added `terminal_id` field to `layout.export` pane nodes, exposing each pane's server-assigned transient terminal id.
...
- Copying text inside a host glass view now shows one "copied to clipboard" confirmation instead of two overlapping boxes; the viewed host keeps its confirmation out of the frame it streams, leaving the viewer's own confirmation, raised when the text reaches the clipboard of the machine being used, as the one shown.
