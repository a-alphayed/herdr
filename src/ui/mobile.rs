...
fn rendered_mobile_workspace_entries(entries: &[WorkspaceListEntry]) -> Vec<MobileWorkspaceEntry> {
    entries
        .iter()
        .enumerate()
...
fn mobile_workspace_entries(app: &AppState) -> Vec<MobileWorkspaceEntry> {
    rendered_mobile_workspace_entries(&workspace_list_entries_expanded(app))
}
...
pub(crate) fn mobile_switcher_target_at(
    ...
    if !agents.is_empty() {
        ...
        if doc_row >= cursor && doc_row < agents_end {
            ...
            return agents.get(idx).and_then(|entry| {
                entry
                    .local_target()
                    .map(|(ws_idx, tab_idx, pane_id)| MobileSwitcherTarget::Agent {
                        ws_idx,
                        tab_idx,
                        pane_id,
                    })
...
fn mobile_switcher_content_height(app: &AppState) -> usize {
    ...
    let spaces_h = 2 + mobile_workspace_entries(app).len() * 2;
    let tabs_h = app
        .active
...
fn render_mobile_switcher_content(
    ...
    let rendered_space_entries = rendered_mobile_workspace_entries(&space_entries);
    for entry in rendered_space_entries {
        ...
        let detail_prefix = if entry.indented {
            let last_child = !next_entry_is_indented_workspace(&space_entries, entry.source_idx);
