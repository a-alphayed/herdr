...
pub(crate) fn confirm_remote_projected_pane_close_inner_rect(area: Rect) -> Option<Rect> {
    centered_popup_rect(area, 78, 9).map(|popup| {
        Rect::new(
            popup.x + 1,
            popup.y + 1,
            popup.width.saturating_sub(2),
...
pub(crate) fn confirm_remote_projected_pane_close_button_rects(inner: Rect) -> (Rect, Rect) {
    let rects = action_button_row_rects(
        inner,
        &[
            ActionButtonSpec {
...
pub(super) fn render_confirm_remote_projected_pane_close_overlay(
    app: &AppState,
    frame: &mut Frame,
    area: Rect,
) {
    let Some(target) = app.pending_remote_projected_pane_close.as_ref() else {
        return;
    };

    super::dim_background(frame, area);
    let Some(inner) = render_modal_shell(frame, area, 78, 9, &app.palette) else {
        return;
    };
    if inner.height < 6 {
        return;
    ...
    render_modal_header(frame, rows[0], "close remote pane", &app.palette);

    let label = truncate_end(&target.label, rows[1].width.saturating_sub(15) as usize);
    frame.render_widget(
        Paragraph::new(format!(" Close remote pane {label}?")).style(
            Style::default()
                .fg(app.palette.text)
                .add_modifier(Modifier::BOLD),
        ),
    ...
    frame.render_widget(
        Paragraph::new(" This closes the remote pane/process on that host.")
            .style(Style::default().fg(app.palette.red)),
        rows[3],
    );

    let (close_rect, cancel_rect) = confirm_remote_projected_pane_close_button_rects(inner);
    render_action_button(
        frame,
        close_rect,
        Some("↵"),
...
pub(crate) fn confirm_remote_projected_tab_close_inner_rect(area: Rect) -> Option<Rect> {
    centered_popup_rect(area, 78, 9).map(|popup| {
        Rect::new(
            popup.x + 1,
            popup.y + 1,
...
pub(crate) fn confirm_remote_projected_tab_close_button_rects(inner: Rect) -> (Rect, Rect) {
    let rects = action_button_row_rects(
        inner,
        &[
            ActionButtonSpec {
...
pub(super) fn render_confirm_remote_projected_tab_close_overlay(
    app: &AppState,
    frame: &mut Frame,
    area: Rect,
) {
    let Some(target) = app.pending_remote_projected_tab_close.as_ref() else {
        return;
    };
    ...
    render_modal_header(frame, rows[0], "close remote tab", &app.palette);

    let label = truncate_end(&target.label, rows[1].width.saturating_sub(14) as usize);
    frame.render_widget(
        Paragraph::new(format!(" Close remote tab {label}?")).style(
            Style::default()
                .fg(app.palette.text)
                .add_modifier(Modifier::BOLD),
        ),
    ...
    frame.render_widget(
        Paragraph::new(format!(
            ...
            target.host, target.session, target.workspace_id
        ))
        .style(Style::default().fg(app.palette.subtext0)),
        rows[2],
    );
    frame.render_widget(
        Paragraph::new(" This closes the remote tab on that host.")
            .style(Style::default().fg(app.palette.red)),
        rows[3],
    );

    let (close_rect, cancel_rect) = confirm_remote_projected_tab_close_button_rects(inner);
    render_action_button(
        frame,
        close_rect,
        Some("↵"),
