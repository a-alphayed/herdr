...
6. Audit `docs/next/CHANGELOG.md` and issue references.
   ...
   - Flag normal commits that use GitHub closing keywords like `fixes #<issue-number>`, `closes #<issue-number>`, or `resolves #<issue-number>`, because normal work lands on `dev` before release and those keywords can close issues too early.
